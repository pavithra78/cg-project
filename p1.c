...
GLint i,k,r,x,y,a=0,b=0,h=0,i=0,w=0,t=0,flag=0,frame=0,x0,x1,bg=1,stars=2,
...
void DEl(GLfloat x, GLfloat y, GLfloat radiusX, GLfloat radiusY) 
{ 
 	float i, x2, y2;  	
 	glPointSize(2.0); 
 	glBegin(GL_TRIANGLE_FAN); 
...
void bu()
{
	...
	glColor3f(0.7,0.4,0.3);//2nd building Ground floor
	glRects(1050,200,1210,800);
	glColor3f(0.0,0.2,0.3);//window
	x=1070;
	for(y=240; y<760; y+=80){
		glRects(x,y,x+40,y+40);
		glRects(x+80,y,x+120,y+40);
	}
	glPopMatrix();
}
void road(){
...
drawText(590, 660, 1,1,1, "RAJASREE JAYARAM");
drawText(590, 620, 1,1,1, "1BI16CS114");
drawText(400.0, 500.0, 1,1,1, "UNDER THE GUIDANCE OF");
drawText(100.0, 390.0, 1,1,1, "Prof. K J Bhanushree");
drawText(100.0, 360.0, 1,1,1, "Assistant Professor, Dept of CSE");
drawText(100.0, 330, 1,1,1,  "Bangalore Institute of Technology, Bangalore");
drawText(650.0, 390.0, 1,1,1, "Prof. K N Prashanth");
drawText(650.0, 360.0, 1,1,1, "Assistant Professor, Dept of CSE");
drawText(650.0, 330, 1,1,1,  "Bangalore Institute of Technology, Bangalore");
drawText(550,160,1,1,1,"Use numbers or on-screen buttons to switch between slides");
drawText(800, 100, 1,1,1,  "PRESS 1 TO START");
glFlush();
}
void buttons(){
	glColor3f(0,0,0);
...
void update2(int f){
	if(frame==2){
		...
		drawText(260,825,0,0,0,"In the rapidly industrializing country, she found herself"); 
		drawText(270,750,0,0,0,"longing for the greenery of the English countryside.");
		glFlush();	
...
void update3(int f){
	...
	glPushMatrix();
		...
		seeds();
	glPopMatrix();
	drawText(290,825,0,0,0,"Hilda imported lupine seeds from her native county.");
	drawText(210,750,0,0,0, "She put them in her pocket and began secretly planting them along the roadside.");
		glFlush();	
...
void update4(int f){
	...
	drawText(250,825,0,0,0,"The lupines flourished, bringing beauty and verdancy to the land.");
	drawText(170,750,0,0,0," Her friends fondly gave her the title of Lupine Lady, and this is the name she is remembered by");
	glFlush();	
...
void mouse(int b,int s,int x,int y){
	if(b==GLUT_LEFT_BUTTON&&s==GLUT_DOWN){
		if((x>1270&&x<1320)&&(y>740&&y<790)) {
			frame--;
			glutPostRedisplay();
		}
		if((x>1365&&x<1420)&&(y>740&&y<790)) {
			frame++;
			glutPostRedisplay();
		}
	}
